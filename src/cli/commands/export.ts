 ...
 * Exports in portable format: { manifest, etag, exportedAt }
 */
function generatePromptFile(exportedPrompt: any): string {
  const manifest = exportedPrompt.manifest;
  const header = `/**
 * ${manifest.name}
 *
 * ⚠️  WARNING: This file is auto-generated by Studio CLI
 * ⚠️  DO NOT EDIT MANUALLY - Changes will be overwritten
 * ⚠️  Edit in Studio UI and run 'skej export' to update
 *
 * Category: ${manifest.category || 'general'}
 * Description: ${manifest.description || 'No description'}
 * Exported: ${exportedPrompt.exportedAt}
 * ETag: ${exportedPrompt.etag}
 */

`;

  const content = `export const ${sanitizeVariableName(manifest.name)} = ${JSON.stringify(
    {
      manifest: exportedPrompt.manifest,
      etag: exportedPrompt.etag,
      exportedAt: exportedPrompt.exportedAt,
    },
    null,
    2
  )};

export default ${sanitizeVariableName(manifest.name)};
`;
 ...
 */
function generateIndexFile(exportedPrompts: any[]): string {
  const header = `/**
 ...
 * Total Prompts: ${exportedPrompts.length}
 */

`;

  const exports = exportedPrompts
    .map((ep) => {
      const fileName = sanitizeFilename(ep.manifest.name);
      const varName = sanitizeVariableName(ep.manifest.name);
...
async function exportSinglePrompt(
  ...
  try {
    ...
    // Write to file in new format: { manifest, etag, exportedAt }
    const content = generatePromptFile(exportedPrompt);
...
export async function exportCommand(options: ExportOptions): Promise<void> {
  try {
    ...
    console.log(`✓ Found ${promptList.data.length} prompts`);
    console.log('');

    if (promptList.data.length === 0) {
      console.log('✅ No prompts to export');
      return;
    }
    ...
    console.log('📥 Downloading prompts...');
    let written = 0;
    const errors: string[] = [];
    const downloadedPrompts: any[] = [];

    for (const item of promptList.data) {
      try {
        // Export prompt
        // Skip version creation in apiMode (prompts loaded at runtime, not for version control)
        process.stdout.write(`   ⬇️  ${item.promptName}...`);
        ...
        // Write to temp file in new format: { manifest, etag, exportedAt }
        const content = generatePromptFile(exportedPrompt);
        ...
        downloadedPrompts.push(exportedPrompt);
        written++;
        process.stdout.write(` ✓\n`);
      } catch (error) {
        process.stdout.write(` ✗\n`);
    ...
    console.log('');
    console.log(`✅ Exported ${written}/${promptList.data.length} prompts to ${outputDir}`);
